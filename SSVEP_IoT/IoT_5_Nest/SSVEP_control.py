...
Control room temperature through the analyzed EEG.(Using Nest)

Use the SVM classifier to classify the results and then control the rise and fall of room temperature.
...
class process():
    ...
    def __init__(self, stack,stim,check,time,sample_count,slice_check):
        ...
        self.url = "http://128.134.65.120:7579/Mobius/kwu-hub/Nest/target_temperature"
        self.headers = {
            ...
            if self.Time[0] != 0:
                ...
                if (self.slice_check[0] == 1):
                    if (self.before[count-1] == 1) :
                        ...
                        self.result = self.cf.nispsvm(data)
                       
                        response = requests.request("GET", self.url+"/latest", headers=self.headers)
                        response11=response.text
                        before = response11[response11.index('<con>')+5:response11.index('</con>')]
                        after = int(int(before) + (-1)**(self.result[0]+1) )
                        
                        if (after>=10) | (after<=40):
                            payload = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<m2m:cin xmlns:m2m=\"http://www.onem2m.org/xml/protocols\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\n    <con>"\
                            +str(after)+"</con>\n</m2m:cin>"

                        response1 = requests.request("POST", self.url, data=payload, headers=self.headers)
                        print('result = ',after)
